...
static PyObject* GfxPolyNew(PyTypeObject* type,
                            PyObject* args, PyObject* kwargs) {
    double gridsize;
    if (!PyArg_ParseTuple(args, "f", &gridsize))
        return NULL;
    gfxpoly_t* poly = gfxpoly_from_fill(NULL, gridsize);
    return WrapGfxPoly(poly);
}

static PyObject* GfxPolyGetAttro(PyObject* self, PyObject* attr) {
    Py_RETURN_NONE;
}
...
static PyObject* GfxPolyRepr(PyObject* self) {
    ...
    PyObject* str = PyUnicode_FromFormat(
            "<gfxpoly %f", poly_obj->poly->gridsize);
...
static PyMethodDef gfxpoly_methods[] = {
    //{"NewPoint", (PyCFunction)(NewPoint),
    //    METH_VARARGS|METH_KEYWORDS, new_point_doc},
    {0, 0, 0, NULL}    // sentinel
};
